...
f_w_news_feature = open('./explain/weibo_news_feature.txt','w')


class Metrics(Callback):
    def __init__(self, platform, alter_params):
        self.log_file = open('./log/Log_DIAL_' + alter_params + '_' + platform +'.txt', 'a')
...
# politifact lr = 0.0002        gosiipcop lr = 0.0015
class DIAL():
    def __init__(self, platform, alter_params, MAX_SENTENCE_LENGTH=50, MAX_COMS_LENGTH=20, MAX_COMS_COUNT=50,
                 MAX_SENTENCE_COUNT=50, USER_COMS=10, lr=0.0002):
        self.model = None
        ...
        self.cp_co_attention_model = None
        self.uc_co_attention_model = None
        self.user_encoder1 = None
        self.user_encoder2 = None
        self.commentEncoder_final = None
        self.uEncoder_final = None
    ...
    def _build_model(self, n_classes=2, embedding_dim=100, embeddings_path=False, aff_dim=80):
        ...
        user_time_emdedding_input = Input(shape=(self.MAX_COMS_COUNT, self.MAX_COMS_DIM_LENGTH,), dtype='float32')
        user_time_emdedding_re = Dropout(0.2, name='user_time_emdedding')(user_time_emdedding_input)

        user_structure_input = Input(shape=(self.MAX_COMS_COUNT, self.MAX_COMS_DIM_LENGTH,), dtype='float32')
        user_structure_re = Dropout(0.2, name='user_structure')(user_structure_input)

        all_user_encoder_final = Add(name='add_u')([user_time_emdedding_re,user_structure_re])
        userEncoder_final = Model([user_time_emdedding_input, user_structure_input], all_user_encoder_final)
        self.uEncoder_final = userEncoder_final

        # Co-attention
        L = LLayer(name="co-attention1")([all_user_encoder_final, content_encoder])
        print(type(L))
        f_w_news_feature.wrote(type(L))
        # L = LLayer(name="co-attention1")([user_time_emdedding_input, content_encoder])
        L_Model = Model([content_input,user_time_emdedding_input,user_structure_input], L)
        ...
        model = Model(inputs=[content_input, user_time_emdedding_input,user_structure_input], outputs=preds)
        model.summary()
        plot_model(model, to_file='CHATT.png', show_shapes=True)
        # parallel_model = keras.utils.multi_gpu_model(GCANmodel, gpus=2)
    ...
    def load_weights(self, saved_model_dir, saved_model_filename):
        with CustomObjectScope({'AttLayer': AttLayer, 'LLayer': LLayer, 'MyTile': MyTile, 'MyAdd': MyAdd}):
            # self.GCANmodel = load_model(os.path.join(saved_model_dir, saved_model_filename))
            self.news_sequence_encoder_input = self.model.get_layer('word_lstm')
            self.news_sequence_encoder_output = self.model.get_layer('content_dropout')
            self.user_encoder2 = self.model.get_layer('user_time_emdedding')
            self.user_structure = self.model.get_layer('user_structure')
            self.uEncoder_final = self.model.get_layer('add_u')
            # self.comment_sequence_encoder = self.model.get_layer('user_comment_dropout')
            # self.commentEncoder_final = self.model.get_layer('hua')
            self.co_attention_model = self.model.get_layer('co-attention1')
    ...
    def _encode_texts(self, texts):
        ...
        for i, text in enumerate(texts):
            ...
            # encoded_text = np.array(pad_sequences(text,
            #     maxlen=self.MAX_COMS_DIM_LENGTH, padding='post', truncating='post', value=0))[:self.MAX_SENTENCE_COUNT]
            encoded_texts[i][:len(encoded_text)] = encoded_text

        return encoded_texts

    def _encode_user_time_emdedding(self, user_time_emdedding):
        """
        Pre process the news content sentences to equal length for feeding to GRU
        :param texts:
        :return:
        """
        encoded_texts = np.zeros((len(user_time_emdedding), self.MAX_COMS_COUNT, self.MAX_COMS_DIM_LENGTH), dtype='float32')
        for i, text in enumerate(user_time_emdedding):
            encoded_text = text[:self.MAX_COMS_DIM_LENGTH]
            # encoded_text = np.array(pad_sequences(text,
            #     maxlen=self.MAX_COMS_DIM_LENGTH, padding='post', truncating='post', value=0))[:self.MAX_SENTENCE_COUNT]
            encoded_texts[i][:len(encoded_text)] = encoded_text
    ...
    def _encode_user_structure(self, user_structure):
        """
        Pre process the news content sentences to equal length for feeding to GRU
        :param texts:
        :return:
        """
        encoded_texts = np.zeros((len(user_structure), self.MAX_COMS_COUNT, self.MAX_COMS_DIM_LENGTH), dtype='float32')
        for i, text in enumerate(user_structure):
            encoded_text = text[:self.MAX_COMS_DIM_LENGTH]
            # encoded_text = np.array(pad_sequences(text,
            #     maxlen=self.MAX_COMS_DIM_LENGTH, padding='post', truncating='post', value=0))[:self.MAX_SENTENCE_COUNT]
            encoded_texts[i][:len(encoded_text)] = encoded_text
    ...
    # def _encode_uc(self, uc):
    #     """
    #     """
    #     encoded_texts = np.zeros((len(uc), self.MAX_COMS_COUNT, self.MAX_COMS_DIM_LENGTH), dtype='float32')
    #     for i, text in enumerate(uc):
    #         encoded_text = text[:self.MAX_COMS_COUNT]
    #         encoded_texts[i][:len(encoded_text)] = encoded_text
    ...
    def train(self, train_x, train_x_u,train_x_us, train_y, val_x, val_x_u,val_x_us, val_y,
        ...
        encoded_train_x_u = self._encode_user_time_emdedding(train_x_u)
        encoded_val_x_u = self._encode_user_time_emdedding(val_x_u)
        encoded_train_x_us = self._encode_user_structure(train_x_us)
        encoded_val_x_us = self._encode_user_structure(val_x_us)
        # encoded_train_uc = self._encode_comments(train_c)
        # encoded_val_uc = self._encode_comments(val_c)
        ...
        self.model.fit([encoded_train_x, encoded_train_x_u,encoded_train_x_us], y=train_y,
                       validation_data=([ encoded_val_x, encoded_val_x_u,encoded_val_x_us], val_y),
                       batch_size=batch_size,
                       epochs=epochs,
                       verbose=1,
                       callbacks=callbacks)

    def predict(self, x, c, x_u,x_us):
        encoded_x = self._encode_texts(x)
        # encoded_c = self._encode_comments(c)
        encoded_x_u = self._encode_user_time_emdedding(x_u)
        encoded_x_us = self._encode_user_structure(x_us)
        return self.model.predict([encoded_x, encoded_x_u,x_us, ])
    ...
    def activation_maps(self, news_article_sentence_list, news_article_comment_list, news_article_user_list,
                        websafe=False):
        """
        ...
        # sentence_level_encoder = Model(inputs=self.news_sequence_encoder_input.input,
        #                                outputs=self.news_sequence_encoder_input.input)

        # sentence_level_weights = sentence_level_encoder.predict(encoded_text)
        sentence_level_weights = encoded_text

        [Wl, Wc, Ws, whs, whc] = self.cp_co_attention_model.get_weights()
